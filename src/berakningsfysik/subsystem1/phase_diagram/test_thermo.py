...
class TestThermodynamics(unittest.TestCase):
    ...
    def test_delta_g_mix_temperature(self):
        ...
        T = 300
        moles=1.0
        S = calculate_delta_s_mix(x, moles=1.0, R=R)
        expected_g = -T * S
        
        calculated_g = calculate_delta_g_mix(x, T, moles=moles, R=R)
    ...
    def test_division_of_three(self):
        ...
        with self.assertRaises(ValueError):
            calculate_delta_s_mix(x, moles=moles, R=R) #Sum != 1

    def test_pure_substance(self):
        x = [1.0, 0.0]
        R = 8.314462618
        moles=1.0
        self.assertEqual(calculate_delta_s_mix(x, moles=moles, R=R), 0.0)
    ...
    def test_negative_mole_fractions(self):
        ...
        moles=1.0
        with self.assertRaises(ValueError):
            calculate_delta_s_mix(x, moles=moles, R=R) #There is a negative number
    ...
    def test_negative_temperature(self):
        ...
        R = 8.314462618
        with self.assertRaises(ValueError):
            calculate_delta_g_mix(x, T, moles=moles, R=R)
    ...
    def test_zero_components(self):
        ...
        moles = 1.0
        with self.assertRaises(ValueError):
            calculate_delta_s_mix(x, moles=moles, R=R) #No components to mix
    ...
    def test_very_high_temperature(self):
        ...
        R = 8.314462618
        expected_g = -T * calculate_delta_s_mix(x, moles=moles, R=R)
